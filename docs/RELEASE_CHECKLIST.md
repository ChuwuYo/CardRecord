...
- [ ] Room schema、迁移测试与备份 schema 3 已核对；零图片/有图片目录布局、图片恢复、旧版本与单 JSON 拒绝路径均通过
...
GitHub Secrets 不是密钥备份。若原签名无法恢复，新包不能覆盖旧安装，卸载又会删除本地数据。只有已经用当前版本验证可读取的完整备份目录才能作为恢复依据；否则必须先提供并验证数据迁移方案，禁止直接建议用户卸载。
