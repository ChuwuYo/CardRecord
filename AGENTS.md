...
- 界面资源、公开 README 与 Changelog 使用用户语言，只描述能力、结果和可采取的操作，不暴露 schema、DTO、URI 等实现术语；源码注释、`docs/Design.md`、本文件与发布清单才记录技术约束。
- 所有界面文案进入 `values/strings.xml` 与 `values-en/strings.xml`，数量文案使用 `plurals`；两种语言的 key、quantity 和格式化参数必须同步。
...
- 新选择的用户图片必须先复制到应用私有目录，再以稳定内容 ID 入库；外部 URI 只用于旧数据幂等迁移。文件暂存、数据库引用与回收必须保证失败最多留下可回收孤儿，不能留下指向半文件的记录。
...
- `ci-release.yml` 以单一 `verify → build-sign → publish` 依赖链执行验证与发布；所有 push / PR / 手动运行先执行无密钥验证，仅验证通过且 tip 仍可发布的 `main` push 或手动运行继续签名并更新滚动测试包（tip 仅为官方 `[skip ci]` 类提交时，允许已通过验证的前一提交完成发布；被更新的实质提交则软跳过签名）。发布纪律见 `docs/RELEASE_CHECKLIST.md`。
