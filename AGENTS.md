...
- 无账号、后端或自动云同步；用户通过 SAF 主动导入/导出备份目录（JSON + 可选图片文件夹），并可导入旧版单 JSON；系统云备份和设备迁移关闭。
...
- 备份 schema 当前导出 `3`，并兼容导入 `1/2`。当前目录始终含 `cardrecord_backup.json`；仅当卡片实际引用图片时创建 `card_images/`，图片内容 ID 同时作为稳定引用与文件名真源，不另存“有无图片”或图片数量等派生字段。导入会核对引用、文件内容并复制回应用私有目录。任何字段、ID、归一化或写入路径变更都要验证导出、REPLACE、MERGE 与旧备份兼容；多表导出取同一事务快照，每个输出文件必须可被本版本导入，导入写入保持单事务。
